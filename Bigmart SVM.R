...
library(e1071)
mymodel_svm = svm(formula = Item_Outlet_Sales ~ .,
                  data = train2,
                  type = 'eps-regression',
                  kernel = 'radial')
...
ypred_svm = predict(mymodel_svm, newdata=test2)

ypred_svm


## generate results CSV file for SVM model 

solution <- data.frame(Item_Identifier = test$Item_Identifier, Outlet_Identifier = test$Outlet_Identifier,ypred_svm)
write.csv(solution, file = "svm.csv", row.names = FALSE)
